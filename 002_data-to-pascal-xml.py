...
"""
This script crawls over training images
not done because dont know how to create bbbox text from lablimg
"""
import cv2
import os
import numpy as np
...
def appendXMLPASCAL(curr_et_object,x1, y1, w, h, filename):
    ...
    ET.SubElement(et_object, "name").text = "windows"
...
# ################################ TRAINING DATA 22 ITEMS ##################################
# # # Run Script for Training data
Train_path = os.path.join(curr_path, "data", "windows_train", "images" )
## comment this out
bbx_gttxtPath = os.path.join(curr_path, "data", "windows_split", "windows_train_bbx_gt.txt" )
readAndWrite(bbx_gttxtPath)


# To folders:
to_xml_folder = os.path.join(curr_path, "data", "tf_windows_train", "annotations", "xmls" )
to_image_folder = os.path.join(curr_path, "data", "tf_windows_train", "images" )
...
train_annotations_index = os.path.join(curr_path, "data", "tf_windows_train", "annotations", "train.txt" )
...
################################ VALIDATION DATA 6 ITEMS ##################################

# Run Script for Validation data
Train_path = os.path.join(curr_path, "data", "windows_val", "images" )
bbx_gttxtPath = os.path.join(curr_path, "data", "windows_split", "windows_val_bbx_gt.txt" )
readAndWrite(bbx_gttxtPath)


# To folders:
to_xml_folder = os.path.join(curr_path, "data", "tf_windows_val", "annotations", "xmls" )
to_image_folder = os.path.join(curr_path, "data", "tf_windows_val", "images" )
...
train_annotations_index = os.path.join(curr_path, "data", "tf_windows_val", "annotations", "val.txt" )
